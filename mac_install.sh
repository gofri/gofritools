...
main() {
    ...
    __RUN_ARGS=''
    get_ip="\$(ifconfig en0 | grep inet | awk '\$1==\"inet\" {print \$2}')"
    add_display="-e DISPLAY=\"$get_ip:0\""
    start_script="$get_ip; ${engine} run --hostname gofritools -it $add_display --detach --privileged -v /:/mnt/root --workdir /mnt/root\$(pwd) --name ${CONTAINER_NAME} ${IMAGE_NAME} bash"
    resume_script="${engine} start ${CONTAINER_NAME}"
    stop_script="${engine} stop ${CONTAINER_NAME}"
    kill_script="${engine} rm ${CONTAINER_NAME}"
    __exec_script="if test -p /dev/stdin; then mode='-t'; else mode='-it'; fi; ${engine} exec \${mode} $add_display --workdir /mnt/root\$(pwd) ${CONTAINER_NAME}"
